...
| FR-005 | Create environment via portal   | As a developer, I want to create a new environment through the Backstage UI by filling a template form sourced live from the GitOps repository so that I do not need to write YAML from scratch and template changes become available without rebuilding the portal image. | High     | Open   |
