...
| FR-005 | Create environment via portal   | As a developer, I want to create a new environment through the Backstage UI by filling a template form sourced live from the GitOps repository, with service enablement controlled by checkboxes and per-service settings shown only for selected services, so that I do not need to write YAML from scratch and template changes become available without rebuilding the portal image. | High     | Open   |
...
| FR-009 | Define environment templates    | As a team lead, I want to define reusable environment templates in Git so that Backstage can discover them live and developers on my team can provision consistent, pre-approved stacks without waiting for a portal image rebuild. | Medium   | Open   |
...
| FR-012 | Override component configuration | As a developer, I want to override default configuration values (e.g., replica count, image tag, approved environment variables such as `account-api` config overrides) for a component in my YAML or approved Backstage template fields so that I can test non-default scenarios per environment. | Medium   | Open   |
...
| C-013 | Live Git-backed template discovery | Backstage scaffolder templates must be discovered from the GitOps repository at runtime via Git/URL-backed catalog locations; image-baked template copies must not be the source of truth for template availability. | Technical | High | Open |
