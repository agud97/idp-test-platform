...
| task-1.5 | BLOCKED | CI workflows created and locally validated; feature branch commit exists, but push to GitHub was rejected because the PAT lacks workflow scope for updating .github/workflows/*.yaml. |
...
## Blockers
- task-1.5: GitHub rejected `git push` for branch `feature/phase-1-foundation` because the Personal Access Token at `/root/codex/token-gh` does not have `workflow` scope required to create or update `.github/workflows/ci.yaml`.
